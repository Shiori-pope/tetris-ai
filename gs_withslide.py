...
import os
# 配置日志记录器
def setup_logger(log_file='yc_finalslide.log', level=logging.INFO):
...
def loss(board_bin):
    ...
    hvar = height.var()
    layers = ((board[:-1] == 0) & (board[1:] == 1)).sum()/10
    subhole = subHoleCount(board)
    max_height = height.max()
    return hvar*0.1 +layers*0.45+ subhole*0.09 + cty_loss*1.1 +max_height*0.2- count*10
...
def enumerate_GameStatus(game):
    ...
    """
    # 保存当前方块状态，以便函数结束时恢复
    original_body = game.current.copy()
...
class GeneticOptimizer:
    ...
    def _evaluate_single_chromosome(self, chromosome):
        ...
        def custom_loss(board_bin):
            ...
            return (hvar * chromosome[0] + 
                   layers * chromosome[1] + 
                   subhole * chromosome[2] + 
                   cty_loss * chromosome[3] + 
                   max_height * chromosome[4] - 
                   count * chromosome[5])
    ...
    def optimize(self):
        ...
        for generation in range(self.generations):
            ...
            print(f"本代平均适应度: {avg_fitness:.2f}, 最佳适应度: {max(fitnesses):.2f}")
            ...
            print(f"第 {generation+1} 代完成，耗时: {time.time() - start_time:.2f}秒")
...
def run_game_with_custom_loss(game, loss_function):
    ...
    while not getattr(game, 'game_over', False) and step < max_steps:
        ...
        if lines == 2:
            ...
            lines_bonus += 5000
        # if lines>1: logger.info(f"bonus line{lines_bonus}")
...
if __name__ == "__main__":
    ...
    optimizer = GeneticOptimizer(population_size=100, generations=100, mutation_rate=0.3)
